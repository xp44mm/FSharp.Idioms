...
type UnionTest(output: ITestOutputHelper) =
    ...
    member this.``UionExample0``() =
        let x = Zero
        let y = Literal.zeroDynamic typeof<UionExample0> :?> UionExample0
        Should.equal x y 

    [<Fact>]
    member this.``UionExample1``() =
        let x = OnlyOne 0
        let y = Literal.zeroDynamic typeof<UionExample1> :?> UionExample1
        Should.equal x y 

    [<Fact>]
    member this.``UionExample2``() =
        let x = Pair (0,"")
        let y = Literal.zeroDynamic typeof<UionExample2> :?> UionExample2
        Should.equal x y 
