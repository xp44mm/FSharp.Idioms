...
type ListTest(output: ITestOutputHelper) =
    ...
    member this.``advanceWhile``() =
        ...
        let y = ls |> List.advanceWhile(fun i -> i < 3)
    ...
    [<Theory>]
    [<Natural 15>]
    member this.``findBalanceLeft``(i: int) =
        let left = "("
        let right = ")"
        let ls =
            [
                [ "(" ], 0, "单个左括号，索引0多余"
                [ "(" ;"("], 0, "两个左括号，索引0就多余了"
                [ "(" ;")" ;"("], 0, "左括号+右括号+左括号，索引0的左括号多余"
                [ ")" ;"(" ;"("], 2, "右括号开头，后面的左括号多余，索引2"
                [ "(" ;")" ;"("], 0, "平衡的括号+一个多余左括号在末尾"
                [ ")" ;"(" ;"("], 2, "多个右括号开头，索引2的左括号多余"
                [ ")" ;"(" ;")" ;"(";"("], 4, "复杂情况：右括号+平衡+左括号"
                [ "(" ;"(" ;"("], 0, "三个左括号，索引0多余"
                [ "(" ;")" ;"(" ;"("], 0, "平衡后多个左括号"
                [ ")" ;"(" ;"(" ;"("], 2, "右括号+三个左括号"
                [ "a" ;"(" ;"("], 1, "非括号字符开头，acc中有a和一个左括号"
                [ "(" ;"a" ;"("], 0, "中间有非括号字符，acc中有右括号和a"
                [ ")" ;"a" ;"b" ;"(" ;"("], 4, "多个非括号字符+两个左括号"
                [ "a" ;"b" ;"c" ;"("], 3, "非括号字符+左括号"
                [ ")" ;"a" ;"(" ;"b" ;"("], 4, "非括号字符在平衡的括号中"
            ]
        let x, e,_ = ls.[i]
        let y = x |> List.findBalanceLeft left right
        output.WriteLine(stringify x.[..y])
        Should.equal y e
        ...
        let nodes =
            Map
                [
                    ...
                    "arrayExprElements", [ "sequentialExpr" ]
                    "arrayTypeSuffix",
                    [
                        "LBRACK"
                        "RBRACK"
                    ...
                    "bindingPattern", [ "headBindingPattern" ]
                    "braceBarExpr",
                    [
                        "STRUCT"
                        "braceBarExprCore"
                    ...
                    "classSpfnMembers", [ "classSpfnMembersAtLeastOne" ]
                    ...
                    "computationExpr", [ "sequentialExpr" ]
                    ...
                    "fileNamespaceSpecList", [ "fileNamespaceSpec" ]
                    "fileNamespaceSpecs",
                    [
                        "fileModuleSpec"
                    ...
                    "interactiveSeparators", [ "interactiveSeparator" ]
                    "interactiveTerminator",
                    [
                        "SEMICOLON_SEMICOLON"
                        "EOF"
                    ...
                    "listExprElements", [ "sequentialExpr" ]
                    "listPatternElements",
                    [
                        "parenPattern"
                        "opt_seps"
                    ...
                    "measureTypeSeq", [ "measureTypePower" ]
                    "memberCore",
                    [
                        "opt_inline"
                        "bindingPattern"
                    ...
                    "moduleDefnsOrExprPossiblyEmpty", [ "moduleDefnsOrExpr" ]
                    "moduleDefnsOrExprPossiblyEmptyOrBlock",
                    [
                        "OBLOCKBEGIN"
                    ...
                    "opt_HIGH_PRECEDENCE_TYAPP", [ "HIGH_PRECEDENCE_TYAPP" ]
                    "opt_OBLOCKSEP", [ "OBLOCKSEP" ]
                    "opt_ODECLEND", [ "ODECLEND" ]
                    "opt_access", [ "access" ]
                    "opt_atomicExprAfterType", [ "atomicExprAfterType" ]
                    ...
                    "opt_equals", [ "EQUALS" ]
                    "opt_explicitValTyparDecls", [ "explicitValTyparDecls" ]
                    "opt_inline", [ "INLINE" ]
                    "opt_inlineAssemblyTypeArg",
                    [
                        "typeKeyword"
                    ...
                    "opt_mutable", [ "MUTABLE" ]
                    "opt_objExprBindings", [ "objExprBindings" ]
                    "opt_objExprInterfaces",
                    [
                        "objExprInterface"
                        "error"
                    ...
                    "opt_topSeparators", [ "topSeparator" ]
                    "opt_typ",
                    [
                        "COLON"
                        "typ"
                    ...
                    "sourceIdentifier", [ "KEYWORD_STRING" ]
                    "staticMemberOrMemberOrOverride",
                    [
                        "STATIC"
                        "MEMBER"
    ...
    member this.``isSuffix``(i: int) =
        let testCases = 
            ...
            (["1";"2";"3";"4"], ["1";"2";"3"], false)          
            |]
        let (suffix, lst, expected) = testCases.[i]
        let result = List.isSuffix suffix lst
        output.WriteLine(sprintf "Case %d: suffix=%A, lst=%A, result=%b, expected=%b" i suffix lst result expected)
