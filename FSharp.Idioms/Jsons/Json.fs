...
type Json =
    ...
    member json.floatValue = json.getValue()
    ...
    member json.stringText = json.getText()
    ...
    member json.elements = json.getElements()

    member json.hasProperty(key: string) =
        match json with
        | Json.Object pairs -> pairs |> List.exists(fst >> (=) key)
        | _ -> false
    ...
    member json.getEntries() =
        ...
        | _ ->
            ArgumentOutOfRangeException "only for Json.Object"
            |> raise

    member json.entries = json.getEntries()

    /// Object.assign：保持先来者顺序，取后来者数值
    member object.assign(entries: seq<string * Json>) =
        [ yield! object.entries; yield! entries ]
        |> List.groupBy fst
    ...
    member json.addProperty(key, value) =
        match json with
        | Json.Object entries -> (key, value) :: entries |> Json.Object
        | _ ->
            ArgumentOutOfRangeException "only for Json.Object"
            |> raise
    ...
    member json.boolValue = json.getBooleanValue()
    ...
    member object.setProperty(propertyName: string, value: Json) =
        ...
        | Json.Object pairs ->
            let pairs =
                pairs
                |> List.map(fun (k, v) ->
                    let v = if k = propertyName then value else v
                    k, v
                )
            Json.Object pairs
        | _ -> failwith "string index is only for object."
    ...
    member array.setElement(index: int, value: Json) =
        ...
        | Json.Array ls ->
            let ls =
                ls
                |> List.mapi(fun i e -> if i = index then value else e)
            Json.Array ls
        | _ -> failwith "int index is only for array."
