...
let tryString =
    fun (ty: Type) ->
        if ty = typeof<string> then
            Some(fun (loop: Loop) (value: obj) -> unbox<string> value |> Json.String)
        else
            None
...
let tryUnit =
    fun (ty: Type) ->
        if ty = typeof<unit> then
            Some(fun (loop: Loop) (value: obj) -> Json.Array [])
        else
            None
...
let tryOption =
    fun (ty: Type) ->
        if
            ty.IsGenericType
            && ty.GetGenericTypeDefinition() = typeof<Option<_>>.GetGenericTypeDefinition()
        then
    ...
    fun (ty: Type) ->
        ...
        | ity ->
            ...
            if
                ...
                && elemType.GetGenericTypeDefinition() = typeof<KeyValuePair<_, _>>.GetGenericTypeDefinition()
            then
                ...
                fun (loop: Loop) (value: obj) ->
                    ...
                    |> List.map(fun kvp ->
                        Json.Array [ (loop: Loop) kty (get_Key kvp); (loop: Loop) vty (get_Value kvp) ]
                    )
                    |> Json.Array
            else
                fun (loop: Loop) (value: obj) ->
...
let tryJson =
    fun (ty: Type) ->
        if
            ...
            && ty = typeof<FSharp.Idioms.Jsons.Json>
        then
            Some(fun (loop: Loop) (value: obj) -> value :?> FSharp.Idioms.Jsons.Json)
        else
            None
