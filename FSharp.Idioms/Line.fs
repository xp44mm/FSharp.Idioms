...
let indent space (code: string) = String.replicate space " " + code
...
/// 各行同时缩进
/// spaces: 行首将要新增的空格个数
let indentMultiline (spaces: int) (codeBlock: string) =
    codeBlock
    |> splitToLines
    |> Seq.map(indent spaces)

/// 缩进输入块的每一行，并合并为一个字符串
let indentAll spaces (blocks: string seq) =
    blocks
    |> Seq.collect(indentMultiline spaces)
    |> String.concat "\r\n"

/// 每行开始的空格数
let startSpaces (lines: string seq) =
    lines
    |> Seq.map(fun line -> line |> Seq.takeWhile((=) ' ') |> Seq.length)
    |> Seq.min
