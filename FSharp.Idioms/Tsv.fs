...
let lineToCell (ln:string) = ln.Split( [| '\t' |], StringSplitOptions.None)

/// get cells
let parseTsv path =
    let lines = File.ReadAllLines(path, Encoding.UTF8)
    lines |> Array.map lineToCell

/// get cells
let parseText (text: string) =
    let text = text.TrimEnd() // Regex.Replace(text, @"\r\n|\r|\n$", "")
    let lines = Line.splitToLines text // Regex.Split(text, @"\r\n|\r|\n")
    lines |> Array.map lineToCell

/// 合成tsv格式的文本
let stringify (cells: string[][]) =
    cells
    |> Array.map(fun cols -> cols |> String.concat "\t")
    |> String.concat "\r\n"
...
let getIntValue (titles: Map<string, int>) title (row: string[]) =
    getValue titles title row |> Int32.Parse
